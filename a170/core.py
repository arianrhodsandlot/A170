...
from a170.helpers import (
    send_stickers_with_keyword_to_chat,
    send_gif_stickers_with_keyword_to_chat
)
...
def reply_sharing(msg):
    ...
    send_stickers_with_keyword_to_chat(sticker_name='发广告的',
                                       chat=chatroom, send_fail_message=True)


def reply_note():
    send_stickers_with_keyword_to_chat(sticker_name='谢谢老板',
                                       chat=chatroom, send_fail_message=True)
